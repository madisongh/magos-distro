...
do_compile() {
    ...
    oe_runmake -j1 -e all TEGRA_ARMABI=${TARGET_ARCH} TARGET_ROOTFS=${STAGING_DIR_TARGET} CUDA_PATH=${STAGING_DIR_NATIVE}/usr/local/cuda-8.0 \
                      CPPFLAGS="${CXX_EXTRA} ${CXXFLAGS} -std=c++11 -I${S}/include/libjpeg-8b-tegra -I${S}/include -I${STAGING_DIR_TARGET}/usr/local/cuda-8.0/include -I${S}/samples/common/algorithm/cuda -I${S}/argus/samples/utils" NVCCFLAGS="--shared --std=c++11" LDFLAGS="-L${STAGING_DIR_TARGET}/usr/local/cuda-8.0/lib ${LDFLAGS}" CFLAGS="${CFLAGS} `pkg-config --cflags opencv`"
}
